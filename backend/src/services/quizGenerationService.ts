...
export async function transcribeVideoUrlToText(videoUrl: string): Promise<string> {
  let videoCleanup: (() => Promise<void>) | undefined;
  const tmpAudio = path.join(os.tmpdir(), `lecture-audio-${randomUUID()}.mp3`);

  try {
    const { localPath: videoPath, cleanup } = await resolveVideoFileForProcessing(videoUrl);
...
export async function generateBulletPointsOnlyForLecture(lectureId: number): Promise<{
  bulletPoints: string[];
}> {
  ...
  if (!transcript) {
    throw new Error('No transcript on this lecture yet. Run Whisper (transcribe) first.');
  }
