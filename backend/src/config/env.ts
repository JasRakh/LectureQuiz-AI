...
const whisperBackendRaw = (process.env.WHISPER_BACKEND || "openai")
  .trim()
  .toLowerCase();
const whisperBackend = whisperBackendRaw === "local" ? "local" : "openai";

const whisperLocalPython =
  (process.env.WHISPER_LOCAL_PYTHON || "python").trim() || "python";
const whisperLocalPythonArgs = (process.env.WHISPER_LOCAL_PY_ARGS || "")
  .trim()
  .split(/\s+/)
  .filter(Boolean);
const whisperLocalModel =
  (process.env.WHISPER_LOCAL_MODEL || "tiny").trim() || "tiny";
const whisperLocalFp16 = ["1", "true", "yes", "on"].includes(
  (process.env.WHISPER_LOCAL_FP16 || "").trim().toLowerCase(),
);
const whisperLocalDevice =
  (process.env.WHISPER_LOCAL_DEVICE || "").trim() || undefined;

const claudeModel =
  (process.env.CLAUDE_MODEL || "claude-sonnet-4-5").trim() ||
  "claude-sonnet-4-5";
