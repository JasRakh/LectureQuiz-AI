...
export default function ProfessorDashboardPage() {
  ...
  const configHints = useMemo(() => {
    ...
    if (!caps.canGenerateBullets) {
      lines.push(
        "Summary bullets need ANTHROPIC_API_KEY (Claude) in backend/.env.",
      );
    }
    if (!caps.canGenerateQuiz) {
      lines.push(
        "Full quiz generation uses Claude after transcription; add ANTHROPIC_API_KEY.",
      );
    }
    return lines;
  }, [caps]);
    ...
    <Box sx={{ display: "flex", flexDirection: "column", gap: 3 }}>
      ...
      >
        <Box>
          ...
          >
            Upload video → Whisper transcript → Claude bullets → full quiz
            (transcribe + bullets + questions).
          </Typography>
        </Box>
        ...
        <Paper sx={{ p: 2, borderRadius: 2, bgcolor: "rgba(15,23,42,0.95)" }}>
          ...
          <Box sx={{ mt: 1, display: "flex", flexWrap: "wrap", gap: 1 }}>
            ...
            <Chip
              size="small"
              label={
                caps.canTranscribe ? "Transcribe: ready" : "Transcribe: blocked"
              }
              sx={{
                fontSize: 11,
                bgcolor: caps.canTranscribe
                  ? "rgba(34,197,94,0.15)"
            ...
            <Chip
              size="small"
              label={
                caps.canGenerateBullets
                  ? "Claude bullets: ready"
                  : "Claude bullets: off"
              }
              sx={{
                fontSize: 11,
                bgcolor: caps.canGenerateBullets
                  ? "rgba(34,197,94,0.15)"
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                <Box sx={{ display: "flex", flexWrap: "wrap", gap: 1 }}>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      void runAction("/generate-bullets", "Bullets")
                    }
                  >
                ...
                <Box>
                  ...
                  <Box
                    component="ul"
                    sx={{ m: 0, pl: 2, color: "#e5e7eb", fontSize: 13 }}
                  >
