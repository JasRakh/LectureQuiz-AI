...
export default function ProfessorDashboardPage() {
  ...
  const [lastResult, setLastResult] = useState<GenerateQuizResponse | null>(null);
  ...
  const handleFileSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (trimmed.length < 2) {
      toast.error('Enter a lecture title (at least 2 characters) before uploading.');
      return;
    }
    const token = window.localStorage.getItem('lecturequiz_token');
    if (!token) {
  ...
  const handleGenerateQuiz = async (lectureId: number) => {
    ...
    setLastResult(null);
    try {
      const res = await fetch(`${apiBase}/lectures/${lectureId}/generate-quiz`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json().catch(() => null);
      ...
      >
        <Box>
          ...
          <Typography variant='body2' sx={{ mt: 0.5, fontSize: 12, color: '#9ca3af' }}>
            Upload an MP4 lecture: the server extracts audio (ffmpeg), transcribes with Whisper,
            then GPT-4o-mini writes bullet points and quiz questions.
          </Typography>
        </Box>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1, minWidth: { md: 280 } }}>
          <Input
      ...
      {lastResult && (
        <Paper sx={{ p: 2.5, borderRadius: 3, bgcolor: 'rgba(15,23,42,0.95)' }}>
          ...
          <Typography variant='caption' sx={{ color: '#9ca3af', display: 'block', mb: 1 }}>
            Quiz ID {lastResult.quiz.id} · {lastResult.quiz.questions.length} questions · fetch via{' '}
            <Typography component='span' variant='caption' sx={{ color: '#a5b4fc' }}>
          ...
          <Box component='ul' sx={{ m: 0, pl: 2.5, color: '#d1d5db', fontSize: 13 }}>
            {lastResult.bulletPoints.map((b, i) => (
              <li key={i}>{b}</li>
            ))}
          </Box>
          <Typography variant='caption' sx={{ color: '#6ee7b7', display: 'block', mt: 2, mb: 0.5 }}>
            Transcript (preview)
          </Typography>
          <Typography
            variant='body2'
          ...
          <Paper sx={{ p: 2.5, borderRadius: 3, bgcolor: 'rgba(15,23,42,0.95)' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
              <Chip
                ...
                size='small'
                sx={{ alignSelf: 'flex-start', bgcolor: '#020617', color: '#9ca3af', fontSize: 11 }}
              />
              <Typography variant='caption' sx={{ color: '#9ca3af' }}>
                Install ffmpeg (e.g. brew install ffmpeg). Without it, audio extraction fails.
              </Typography>
              <Chip
              ...
              />
              <Typography variant='caption' sx={{ color: '#9ca3af' }}>
                Whisper handles speech-to-text; gpt-4o-mini builds bullets and multiple-choice
                questions. Audio must stay under the Whisper 25MB limit after compression.
              </Typography>
            </Box>
          </Paper>
        </Grid>
