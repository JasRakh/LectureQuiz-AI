...
export default function ProfessorDashboardPage() {
  ...
  const refreshList = useCallback(async () => {
    ...
    setLoadingList(true);
    try {
      const res = await fetch(`${apiBase()}/lectures`, { headers: authHeaders });
      if (res.status === 401) {
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
      ...
      {caps && (
        <Paper sx={{ p: 2, borderRadius: 2, bgcolor: 'rgba(15,23,42,0.95)' }}>
          ...
          {configHints.length > 0 && (
            <Alert severity="warning" sx={{ mt: 2, fontSize: 12, bgcolor: 'rgba(250,204,21,0.08)' }}>
              {configHints.map((h) => (
                <Typography key={h} variant="body2" sx={{ fontSize: 12, mb: 0.5 }}>
                  {h}
                </Typography>
              ))}
            </Alert>
      ...
      <Grid container spacing={2}>
                ...
                {lectures.map((l) => (
                  <Paper
                    ...
                    sx={{
                      ...
                      border:
                        selectedId === l.id ? '1px solid rgba(56,189,248,0.4)' : '1px solid transparent',
                    }}
                  >
        ...
        <Grid size={{ xs: 12, md: 7 }}>
          <Paper sx={{ p: 2.5, borderRadius: 3, bgcolor: 'rgba(15,23,42,0.95)', minHeight: 360 }}>
            {!selectedId && (
